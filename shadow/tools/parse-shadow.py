...
def single_parse(args):
    ...
    if len(sdata) > 0: dump(args, sdata, SHADOWJSON, False)
    if len(tdata) > 0: dump(args, tdata, TORJSON, False)
    if len(fdata) > 0: dump(args, fdata, FILETRANSFERJSON, False)
    if len(tgendata) > 0: dump(args, tgendata, TGENJSON, False)
...
def worker_main_helper(args, partsQ, helperfunc, dumpfilename):
    ...
    while stopcount < NUMFILTERWORKERS+1:
        for partbatch in iter(partsQ.get, 'STOP'):
            ...
            partsQ.task_done()
        stopcount += 1
    if len(data) > 0: dump(args, data, dumpfilename, False)
...
def dump(args, data, filename, compress=True):
    print 'dumping data to {0}'.format(filename)
    ...
    if compress: # inline compression
        ...
        #ddp = Popen(["dd", "status=noxfer", "of={0}".format(path)], stdin=xzp.stdout)
        ddp = Popen(["dd", "of={0}".format(path)], stdin=xzp.stdout)
